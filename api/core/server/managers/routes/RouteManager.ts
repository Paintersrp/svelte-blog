...
export class RouteManager {
   ...
   * @public
   */
  public initializeManagementRoutes(
    app: Koa,
    router: Router,
    lifecycleManager: LifecycleManager
  ) {
    router.get(
      `/sys/start`,
      this.checkAdminRole(),
      lifecycleManager.start.bind(this)
    );
    router.get(
      `/sys/stop`,
      this.checkAdminRole(),
      lifecycleManager.gracefulShutdown.bind(this)
    );
  ...
  private generateSchemaFromModel(model: ModelStatic<any>): Yup.Schema {
    ...
    for (const [attribute, type] of Object.entries(model.getAttributes())) {
      const sequelizeType = type.type as
      ...
      if (sequelizeType.options?.length) {
        fieldSchema = (
          fieldSchema as Yup.NumberSchema<number> | Yup.StringSchema<string>
        ).max(
